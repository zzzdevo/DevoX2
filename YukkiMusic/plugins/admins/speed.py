...
@app.on_message(
    command(["/cspeed", "/speed", "/cslow", "/slow", "/playback", "/cplayback","speed","خێرای"])
    & ~filters.private
    & ~BANNED_USERS
)
@AdminRightsCheck
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    mystic = await CallbackQuery.edit_message_text(
        text=_["admin_40"].format(CallbackQuery.from_user.mention),
    )
    try:
        ...
        return await mystic.edit_text(_["admin_41"], reply_markup=close_markup(_))
    if chat_id in checker:
        checker.remove(chat_id)
    await mystic.edit_text(
        text=_["admin_42"].format(speed, CallbackQuery.from_user.mention),
